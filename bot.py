...
KNOWLEDGE_FILE = "/app/knowledge.json"
...
def update_report(key, status, issue=None):
    REPORT[key] = {"status": status, "issue": issue}


def format_status(key):
...
async def send_daily_report(app):
    ...
    text += "📋 Задачи дня:\n"
    text += f"{format_status('records_tomorrow')} Записи на завтра проверены\n"
    text += f"{format_status('free_slots')} Свободные окошки опубликованы\n"
    text += f"{format_status('unconfirmed_17')} Неподтверждённые — написала в 17:00\n"
    text += f"{format_status('calls_1930')} Звонки в 19:30\n\n"
    text += "🔧 Вечерний чеклист:\n"
    text += f"{format_status('yclients_done')} Все клиенты пробиты в YClients\n"
    text += f"{format_status('checks_done')} Чеки сверены\n"
    text += f"{format_status('integrations_ok')} Интеграции работают\n"
    text += f"{format_status('systems_ok')} Все системы работают\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("waiting_issue"):
        ...
        update_report(key, False, issue=user_message)
    ...
    try:
        ...
        response = client.messages.create(
            ...
            max_tokens=1000,
            system=BASE_PROMPT + load_knowledge(),
            messages=[{"role": "user", "content": f"Сообщение от клиента:\n\n{user_message}"}]
        )
        await update.message.reply_text(f"💬 {response.content[0].text}")
...
async def teach(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_knowledge(rule)
    await update.message.reply_text(f"✅ Запомнила: {rule}")
